...
def get_force_warped_to_img(img, pressure, homography, weak=False):
        if weak:
            # print('Trying to get pressure from a weakly labeled sequence', self.seq_path)
            return np.zeros((img.shape[0], img.shape[1]))

        # if homography is not a matrix full of -1s
        if np.all(homography != -1):
            force_warped = cv2.warpPerspective(pressure, homography, (img.shape[1], img.shape[0]))
        else:
            force_warped = np.zeros((img.shape[0], img.shape[1]))

        return force_warped

def get_force_overlay_img(img, pressure, homography):
    # takes in kPa pressure and undistorted image and returns an image with the pressure overlayed on it
    force_warped = cv2.warpPerspective(pressure, homography, (img.shape[1], img.shape[0]))
    force_color_warped = pressure_to_colormap(force_warped, colormap=cv2.COLORMAP_INFERNO)

    return cv2.addWeighted(img, 1.0, force_color_warped, 1.0, 0.0)

def get_sensel_scale(sensel_corners, output_shape):
    # Magic function to get the scale of the sensel image
