...
class I3DEncoder(torch.nn.Module):
    def __init__(self, modality):
        ...
        self.out_channels = [3 * self.num_frames,
                             64 * self.num_frames // 2,
                             192 * self.num_frames // 2,
                             480 * self.num_frames // 2,
                             832 * self.num_frames // 4,
                             1024 * self.num_frames // 8]
    ...
    def load_pretrained_weights(self):
        if self.modality == 'rgb':
            weights = 'data/pretrained/model_rgb.pth'
        elif self.modality == 'flow':
            weights = 'data/pretrained/model_flow.pth'

        print('Loading pretrained I3D weights', weights)
        self.load_state_dict(torch.load(weights))

    def load_state_dict(self, state_dict, **kwargs):
        state_dict.pop('conv3d_0c_1x1.conv3d.weight', None)
        state_dict.pop('conv3d_0c_1x1.conv3d.bias', None)
        super().load_state_dict(state_dict, **kwargs)

    def forward(self, inp):
        # Preprocessing
        features = list()   # this is what we'll return

        flattened = inp.view(inp.shape[0], inp.shape[1] * inp.shape[2], inp.shape[3], inp.shape[4])
        features.append(inp)    # capture original resolution
        out = self.conv3d_1a_7x7(inp)

        flattened = out.view(out.shape[0], out.shape[1] * out.shape[2], out.shape[3], out.shape[4])
        features.append(flattened)    # capture downsample
